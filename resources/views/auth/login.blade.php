...
<head>
    ...
    <title>Masuk - E-Booking Rumah Anno 1925</title>
...
<body class="min-h-screen bg-slate-50 text-slate-800 flex flex-col justify-between p-4 sm:p-6 lg:p-8 relative overflow-x-hidden antialiased">
        ...
        <div class="overflow-hidden rounded-3xl sm:rounded-[32px] border border-white bg-white/90 shadow-[0_20px_60px_-15px_rgba(16,185,129,0.12)] backdrop-blur-xl grid lg:grid-cols-12">
            ...
            <div class="hidden lg:flex lg:col-span-5 flex-col justify-between bg-gradient-to-br from-emerald-800 via-teal-800 to-slate-900 p-10 text-white relative overflow-hidden">
                ...
                <div>
                    <div class="inline-flex items-center gap-2 px-3.5 py-1.5 text-xs font-semibold text-emerald-200 bg-white/10 rounded-full border border-white/15 backdrop-blur-md">
                        <i class="fa-solid fa-landmark text-emerald-300"></i>
                        <span>E-Booking Portal</span>
                    </div>

                    <h2 class="mt-8 text-3xl font-extrabold leading-tight tracking-tight">
                        Kelola Booking Venue Heritage Anda
                    </h2>
                    <p class="mt-4 text-sm text-emerald-100/80 leading-relaxed font-light">
                        Akses sistem terpadu untuk memantau status pengajuan, mengecek jadwal ruangan, dan mengelola reservasi di Rumah Anno 1925.
                    </p>
                </div>
                <div class="space-y-3 my-6">
                    <div class="flex items-center gap-3 p-3.5 rounded-2xl bg-white/10 border border-white/10 backdrop-blur-md">
                        ...
                        <span class="text-xs font-medium text-emerald-50">Cek ketersediaan secara real-time</span>
                    </div>
                    <div class="flex items-center gap-3 p-3.5 rounded-2xl bg-white/10 border border-white/10 backdrop-blur-md">
                        <div class="w-8 h-8 rounded-xl bg-amber-400/20 flex items-center justify-center text-amber-300">
                            <i class="fa-solid fa-shield-halved text-xs"></i>
                        </div>
                        <span class="text-xs font-medium text-emerald-50">Sistem persetujuan multi-role aman</span>
                    </div>
                </div>
            ...
            <div class="lg:col-span-7 p-6 sm:p-10 lg:p-12 flex flex-col justify-center bg-white/80">
                <div class="text-center sm:text-left mb-8">
                    ...
                    <p class="mt-2 text-xs sm:text-sm text-slate-500">Masukkan akun Anda untuk melanjutkan ke dashboard</p>
                </div>
                ...
                <form action="{{ url('/login') }}" method="POST" class="space-y-5">
                    ...
                    <div>
                        <label for="email" class="mb-2 block text-xs sm:text-sm font-semibold text-slate-700">
                            Alamat Email
                        </label>
                        <div class="relative">
                            ...
                            <input type="email" id="email" name="email" value="{{ old('email') }}" required autofocus
                                ...
                                class="w-full rounded-2xl border {{ $errors->has('email') ? 'border-red-300 focus:border-red-500 focus:ring-red-100' : 'border-slate-200 focus:border-emerald-500 focus:ring-emerald-500/10' }} bg-slate-50/70 py-3.5 pl-11 pr-4 text-xs sm:text-sm text-slate-800 placeholder-slate-400 outline-none transition-all focus:bg-white focus:ring-4">
                        </div>
                        @error('email')
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="password" id="password" name="password" required
                                ...
                                class="w-full rounded-2xl border border-slate-200 bg-slate-50/70 py-3.5 pl-11 pr-11 text-xs sm:text-sm text-slate-800 placeholder-slate-400 outline-none transition-all focus:border-emerald-500 focus:bg-white focus:ring-4 focus:ring-emerald-500/10">
                    ...
                    <div class="text-center text-xs sm:text-sm text-slate-500 pt-4 border-t border-slate-100">
                        ...
                        <a href="{{ route('register') }}"
                            class="ml-1 font-bold text-emerald-600 hover:text-emerald-700 hover:underline transition-colors">Daftar sekarang</a>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
        ...
        document.querySelector('form').addEventListener('submit', function () {
            ...
            document.getElementById('loginBtnText').textContent = 'Memproses...';
            btn.disabled = true;
        });
    </script>
</body>
